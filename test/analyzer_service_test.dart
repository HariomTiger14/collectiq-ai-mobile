...
void main() {
  ...
  group('MockAnalyzerProvider', () {
    test('consumes the backend analyzer contract when configured', () async {
      ...
      expect(
        response.rawProviderPayload['analysisPath'],
    ...
    test('missing endpoint path uses local mock fallback safely', () async {
      final provider = MockAnalyzerProvider(
        analysisProvider: const _SuccessfulLegacyProvider(),
        backendClient: const _FailingBackendClient(
          AiBackendClientErrorType.endpointMissing,
        ),
        localFallbackReason: 'backend_endpoint_not_configured',
      );

      final response = await provider.analyze(
        const AnalyzerRequest(imagePath: '/tmp/local-card.jpg'),
      );
    ...
    test(
      'backend failure exposes diagnostic reason without local fallback',
      () async {
        final provider = MockAnalyzerProvider(
          analysisProvider: const _SuccessfulLegacyProvider(),
          backendClient: const _FailingBackendClient(
        ...
        await expectLater(
          ...
          throwsA(
            isA<AnalyzerException>()
                ...
                .having(
                  (error) => error.details['backendClientErrorType'],
                  'backend client error type',
                  'timeout',
                ),
          ),
        );
    ...
    test('backend response mapping handles missing fields safely', () async {
      final backendResponse = AiBackendAnalysisResponse.fromJson({
        'title': 'Sparse Backend Result',
      });
      final response = backendResponse.toScanResult(
        thumbnail: '/tmp/sparse.jpg',
      );

      expect(response.title, 'Sparse Backend Result');
      ...
      expect(response.thumbnail, '/tmp/sparse.jpg');
      expect(
    ...
    test('backend request serializes multi-photo roles', () {
      final request = AiBackendAnalysisRequest(
        imagePath: '/tmp/coin-front.jpg',
        imageSource: 'gallery',
      ...
      expect((images.first as Map<String, dynamic>)['imageRole'], 'front');
      expect((images.last as Map<String, dynamic>)['imageRole'], 'back');
    });

    test('backend response maps photo and valuation metadata', () {
      final backendResponse = AiBackendAnalysisResponse.fromJson({
        'id': 'coin-test',
        'title': r'Australian $2 Coin',
        'category': 'Coin',
        'estimatedValue': 0,
        ...
        'confidence': 58,
        'condition': 'Unknown',
        'recommendation': 'Add reverse photo.',
        'rawProviderPayload': {
          'photosUsed': 1,
          'photoRoles': ['front'],
        },
...
class _FailingBackendClient implements AiBackendClient {
  const _FailingBackendClient(this.errorType);

  final AiBackendClientErrorType errorType;
  ...
  Future<AiBackendAnalysisResponse> analyze(
    ...
    switch (errorType) {
      ...
      case AiBackendClientErrorType.invalidEndpoint:
        throw AiBackendClientException.invalidEndpoint('Invalid endpoint.');
      ...
      case AiBackendClientErrorType.invalidImagePayload:
        throw AiBackendClientException.invalidImagePayload('Invalid image.');
      ...
      case AiBackendClientErrorType.unsupportedProvider:
        throw AiBackendClientException.unsupportedProvider('Backend AI');
    }
  }
}
